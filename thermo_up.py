...
# 파일 불러오기 (엑셀 파일)
excel_file = './general_que_up.xlsx'
...
# Streamlit app
st.markdown("""
    <h1 style='text-align: center;'>AI 기반 열 및 통계물리 피드백 시스템</h1>
...
# 동의 후 이전 데이터 불러오기
    ...
    if st.button("동의"):
        ...
        select_query = """
        SELECT id, domain_1_done, domain_2_done, domain_3_done
        FROM mid_term
        ...
        if result:
            ...
            if st.session_state.completed_domains >= total_domains:
                ...
                if load_data_choice == "예":
                    ...
                    # 새로운 레코드 생성
                    insert_query = """
                    INSERT INTO mid_term (name, email, date) VALUES (%s, %s, NOW())
                    """
